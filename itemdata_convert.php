...
$fp = fopen( 'input\ItemData.txt' , 'r');
fseek($fp, 1); //skip unicode special chars

$fp2 = fopen( 'output\ItemData.txt' , 'w');
fwrite($fp2, chr(255).chr(254)); //add unicode special chars (FF FE)

while (!feof($fp)) {
	$str = substr( fgets($fp), 1) . chr(00); //fix unicode string
	$line = rtrim(iconv('UCS-2LE', 'ASCII', $str));
	$line = preg_replace('/\/\*.*?\*\//', '', $line); //remove inline comments
	$line = preg_replace("/[\t]+/", "\t", $line); //remove dup tabs
	$line = preg_replace('/[\s]*=[\s]*/', '=', $line); //normalize

	$params = split("\t", $line);
	$param_last = count($params)-1;
	if ($params[0] == 'item_begin') {
		$item_id = (int)$params[2];
		
		if ( in_array($item_id, $spellbooks))
		{
			list(,$consume_type) = split('=', $params[12]); //consume_type -> consume_type_stackable

			$start = strpos($str, iconv('ASCII', 'UCS-2LE', "\tconsume_type") )+2;
			$end = $start;
			while ( substr($str, $end, 2) !== chr(9).chr(00) ) { //x09 x00 == \t
				$end = $end+2;
			}
			$str = substr_replace($str, iconv('ASCII', 'UCS-2LE', "consume_type=consume_type_stackable") , $start, $end-$start);
			echo "[{$item_id}] fixed. \r\n";
